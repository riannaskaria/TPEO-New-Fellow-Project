...
const AddFriends = ({onLogout}) => {
  ...
  const sendFriendRequest = async (userId) => {
    try {
      // Get the target user
      const userResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${userId}`);
      if (!userResponse.ok) {
      ...
      const updateResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${userId}`, {
        method: 'PUT',
