...
const Friends = ({onLogout}) => {
  ...
  useEffect(() => {
    ...
    const fetchFriends = async () => {
      try {
        const response = await authService.fetchWithAuth('tpeo-new-fellow-project.vercel.app/users');
        if (response.ok) {
  ...
  const fetchFriendEvents = async () => {
    ...
    try {
      ...
      for (const friendId of currentUser.friends) {
        const friendResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${friendId}`);
        ...
        if (friend.savedEvents?.length > 0) {
          for (const eventId of friend.savedEvents) {
            const eventResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/events/${eventId}`);
  ...
  const acceptFriendRequest = async (userId) => {
    try {
      ...
      const updateResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${currentUser._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const otherUserResponse = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${userId}`);
      ...
      await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${userId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRemoveFriend = async (friendId) => {
    try {
      ...
      const updateCurrentUserRes = await authService.fetchWithAuth(
        `tpeo-new-fellow-project.vercel.app/users/${currentUser._id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      const friendRes = await authService.fetchWithAuth(`tpeo-new-fellow-project.vercel.app/users/${friendId}`);
      ...
      const updateFriendRes = await authService.fetchWithAuth(
        `tpeo-new-fellow-project.vercel.app/users/${friendId}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
              ...
              {friendRequestsData.map(user => (
                <div key={user._id} className="request-card">
                  <div className="user-info">
                    <img
                      src={user.profilePicture ? `tpeo-new-fellow-project.vercel.app/users/image/${user.profilePicture}` : "/assets/profile.svg"}
              ...
              <div className="friend-grid">
                {friends.map(friend => (
                  ...
                  >
                    ...
                    >
                      <img
                        src={friend.profilePicture ? `tpeo-new-fellow-project.vercel.app/users/image/${friend.profilePicture}` : "/assets/profile.svg"}
